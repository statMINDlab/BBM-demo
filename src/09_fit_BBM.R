...
Sys.setenv(OMP_NUM_THREADS = as.character(floor(48/length(subject_ids))))
...
prior_path <- if (nIC == 0) {
    ...
    file.path(dir_data, "priors", "MSC", "prior_combined_MSC_noGSR.rds")
} else if (nIC == 2) {
...
for (subject_id in subject_ids){
  ...
  Sys.setenv(TMPDIR = td)
    ...
    for (sesid in session_id) {
        ...
        base_name <- paste0("sub-", subject_id, "_ses-", sesid, "_brainmap")
        ...
        if (smoothing) {
          ...
          # check if smoothed results already exist
          bold_fname <- file.path("/N/project/BayesianBrainMapping/smoothed_bold", paste0("sub-", subject_id, "/rfMRI_", sesid, "_", encoding, "_Atlas_MSMAll_hp2000_clean_smoothed-", smoothing, "mm.dtseries.nii"))
          if (all(file.exists(bold_fname))) {
            cat("  Loading existing smoothed results for subject", subject_id, "session", sesid, "\n")
            bold_cifti <- lapply(bold_fname, function(x) {
              read_cifti(x)
            })
            ...
            # Define scrubbing indices (example: scrubbing last 600 seconds)
            cat("Running smoothing for bold timeseries...", "\n")
        ...
        ### Start debug, load cifti smoothed from storage ###################
        bold_cifti <- lapply(bold_fname, read_cifti, brainstructures = c("left", "right")) # cortical only
        ### End debug section ####
